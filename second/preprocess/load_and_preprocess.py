...
def parse_purchase_history(record):
    """解析JSON格式的购买记录
    {"avg_price":4194,"categories":"智能手表","items":[{"id":1461},{"id":7826},{"id":8220},{"id":2159}],"payment_method":"信用卡","payment_status":"已支付","purchase_date":"2024-09-27"}
    """
    try:
        data = json.loads(record.replace("'", '"'))
        return pd.Series({
            'avg_price': data.get('avg_price', 0),
            'category': data.get('categories', 'unknown'),
...
def load_data(file_pattern):
    ...
    for file in glob.glob(file_pattern):
        for chunk in pd.read_csv(file, chunksize=10000):
            # 解析关键字段
            chunk[['avg_price', 'categories', 'items_count', 'items', 'payment_method', 'payment_status', 'purchase_date']] = chunk['purchase_history'].apply(parse_purchase_history)
    ...
    for file in file_progress:
        try:
            ...
            for batch in parquet_file.iter_batches(batch_size=1250000):
                ...
                df[['avg_price', 'categories', 'items_count', 'items', 'payment_method', 'payment_status', 'purchase_date']] = df['purchase_history'].apply(parse_purchase_history)
